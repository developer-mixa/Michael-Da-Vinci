...
async def finding(message: Message, state: FSMContext):
    ...
    if message_text == STOP:
        ...
        liked_data = LikeUserData(user_id=message.from_user.id, liked_user_id=liked_user_id, action=LIKE_ACTION)
...
async def __push_search_answer(response: AcquaintanceResponse, message: Message, state: FSMContext):
    ...
    if search_response == AcquaintanceResponseStatus.FOUND:
        ...
        await message.answer_photo(photo=image_input_file, caption=msg.USER_INFO_TEMPLATE.format(
            name=found_user_data['name'],
            age=found_user_data['age'],
            description=found_user_data['description']
...
async def __push_liked_answer(response: AcquaintanceResponse, message: Message, state: FSMContext):
    ...
    if liked_response == LikedResponseStatus.MUTUALLY:
        ...
        await message.answer(msg.MUTUALL_LIKE_TEMPLATE.format(username=liked_username), reply_markup=ReplyKeyboardRemove())
        await bot.send_message(mutually_data['liked_user_id'], msg.MUTUALL_LIKE_TEMPLATE.format(username=message.from_user.username))
        await state.clear()
